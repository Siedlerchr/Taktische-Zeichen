...
function renderAll() {
    if (!fs.existsSync(path.join(__dirname, "96x96"))) {
        fs.mkdirSync(path.join(__dirname, "96x96"));
    }
    ...
    for (var i = 0; i < config.sets.length; i++) {
        ...
        if (!fs.existsSync(path.join(__dirname, '96x96', set.name))) {
            fs.mkdirSync(path.join(__dirname, '96x96', set.name));
        }
        ...
        for (var x = 0; x < set.symbols.length; x++) {
            ...
            for (var variant in symbol.variants) {
                ...
                jobs.push({
                    ...
                    png64File: path.join(__dirname, '96x96', set.name, symbol.variants[variant] + ".png")
    ...
    async.eachLimit(jobs, 10, function (job, callback) {
        ...
        render.on('close', function (code) {
            ...
            async.eachSeries([
                    ...
                    {filename: job.png64File, factor: "9.375%"}
                ],
                function (resizeJob, resizeFinished) {
                    console.log("Resizing " + job.pngFile + " by " + resizeJob.factor + " to " + resizeJob.filename);
                    var resize = spawn("convert", [job.pngFile, "-resize", resizeJob.factor, resizeJob.filename]);
